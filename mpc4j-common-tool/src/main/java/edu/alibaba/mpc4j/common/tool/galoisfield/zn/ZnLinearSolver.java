 ...
 * @date 2022/7/6
 */
public class ZnLinearSolver {
    /**
     * Zn运算库
     */
    private final Zn zn;

    public ZnLinearSolver(Zn zn) {
        this.zn = zn;
    }
    ...
    private int rowEchelonForm(BigInteger[][] lhs, BigInteger[] rhs, boolean breakOnUnderDetermined) {
        ...
        for (int iColumn = 0, to = Math.min(nRows, nColumns); iColumn < to; ++iColumn) {
            ...
            for (int iRow = row + 1; iRow < nRows; ++iRow) {
                BigInteger alpha = zn.div(lhs[iRow][iColumn], lhs[row][iColumn]);
                if (rhs != null) {
                    rhs[iRow] = zn.sub(rhs[iRow], zn.mul(rhs[row], alpha));
                }
                if (!alpha.equals(BigInteger.ZERO)) {
                    for (int iCol = iColumn; iCol < nColumns; ++iCol) {
                        lhs[iRow][iCol] = zn.sub(lhs[iRow][iCol], zn.mul(alpha, lhs[row][iCol]));
                    }
                }
            }
        }
    ...
    public LinearSolver.SystemInfo solve(BigInteger[][] lhs, BigInteger[] rhs, BigInteger[] result, boolean solveIfUnderDetermined) {
        ...
        if (nUnderDetermined == 0 && nColumns <= nRows) {
            for (int i = nColumns - 1; i >= 0; i--) {
                ...
                for (int j = i + 1; j < nColumns; j++) {
                    sum = zn.add(sum, zn.mul(result[j], lhs[i][j]));
                }
                result[i] = zn.div(zn.sub(rhs[i], sum), lhs[i][i]);
            }
            return Consistent;
        }
        ...
        for (int iColumn = 0, to = Math.min(nRows, nColumns); iColumn < to; ++iColumn) {
            ...
            BigInteger valInv = zn.inv(val);

            for (int i = iColumn; i < nColumns; i++) {
                row[i] = zn.mul(valInv, row[i]);
            }

            rhs[iRow] = zn.mul(rhs[iRow], valInv);
            ...
            for (int i = 0; i < iRow; i++) {
                ...
                for (int j = iColumn; j < nColumns; ++j) {
                    pRow[j] = zn.sub(pRow[j], zn.mul(v, row[j]));
                }
                rhs[i] = zn.sub(rhs[i], zn.mul(rhs[iRow], v));
            }
