 ...
 * @date 2021/09/11
 */
public class ZnMaxLisFinder implements MaxLisFinder {
    /**
     * Z_p域的质数p
     */
    ...
    public ZnMaxLisFinder(BigInteger N, BigInteger[][] matrix) {
        this.N = N;
        ...
        IntStream.range(0, m)
                .forEach(rowIndex -> IntStream.range(0, n)
                        .forEach(columnIndex -> copiedMatrix[rowIndex][columnIndex] = matrix[rowIndex][columnIndex].mod(N))
                );
    ...
    private void pivot(int p) {
        for (int i = p + 1; i < m; i++) {
            BigInteger alpha = copiedMatrix[i][p].multiply(copiedMatrix[p][p].modInverse(N)).mod(N);
            for (int j = p; j < n; j++) {
                copiedMatrix[i][j] = copiedMatrix[i][j].subtract(alpha.multiply(copiedMatrix[p][j])).mod(N);
            }
        }
    }
