...
class H3TcGctZpmOvdm<T> extends AbstractZpmOvdm<T> {
    /**
    ...
    H3TcGctZpmOvdm(EnvType envType, BigInteger prime, int n, byte[][] keys, int polyDegree) {
        super(envType, prime, n, getLm(n) + getRm(n), polyDegree);
        lm = getLm(n);
        rm = getRm(n);
    ...
    public BigInteger[] decode(BigInteger[][] storage, T key) {
        ...
        // 三个哈希结果一定不同，计算3次求和
        for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
            values[coefIndex] = zp.add(values[coefIndex], storage[hValues[0]][coefIndex]);
            values[coefIndex] = zp.add(values[coefIndex], storage[hValues[1]][coefIndex]);
            values[coefIndex] = zp.add(values[coefIndex], storage[hValues[2]][coefIndex]);
            for (int rmIndex = 0; rmIndex < rm; rmIndex++) {
                if (rxBinary[rmIndex]) {
                    values[coefIndex] = zp.add(values[coefIndex], storage[lm + rmIndex][coefIndex]);
                }
            }
        }
        return values;
    ...
    @Override
    public ZpmOvdmFactory.ZpmOvdmType getZpmOvdmType() {
        return ZpmOvdmFactory.ZpmOvdmType.H3_SINGLETON_GCT;
    }
    ...
    public BigInteger[][] encode(Map<T, BigInteger[]> keyValueMap) throws ArithmeticException {
        ...
        // 将矩阵拆分为L || R
        BigInteger[][] leftStorage = new BigInteger[lm][polyDegree];
        BigInteger[][] rightStorage = new BigInteger[rm][polyDegree];
        ...
        Map<T, BigInteger[]> removedDataInnerProductMap = removedDataStack.stream()
                .collect(Collectors.toMap(Function.identity(), removedData -> {
                    ...
                    BigInteger[] rightInnerProducts = innerProduct(rightStorage, rx);
                    BigInteger[] values = keyValueMap.get(removedData);
                    return sub(values, rightInnerProducts);
                }));
        while (!removedDataStack.empty()) {
            ...
            BigInteger[] innerProducts = removedDataInnerProductMap.get(removedData);
            // 三个顶点一定不相同
            fullDistinctVertices(leftStorage, innerProducts, vertex0, vertex1, vertex2, removedData);
        }
        // 左侧矩阵补充随机数
        for (int vertex = 0; vertex < lm; vertex++) {
            if (leftStorage[vertex][0] == null) {
                for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++) {
                    leftStorage[vertex][coefIndex] = zp.createNonZeroRandom(secureRandom);
                }
            }
        }
        // 更新矩阵
    ...
    private void fullDistinctVertices(BigInteger[][] leftMatrix, BigInteger[] innerProduct,
        ...
        if (leftMatrix[vertex0][0] == null && leftMatrix[vertex1][0] == null && leftMatrix[vertex2][0] == null) {
            // 0、1、2都为空
            for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
                leftMatrix[vertex0][coefIndex] = zp.createNonZeroRandom(secureRandom);
                leftMatrix[vertex1][coefIndex] = zp.createNonZeroRandom(secureRandom);
                leftMatrix[vertex2][coefIndex] = zp.sub(zp.sub(innerProduct[coefIndex],
                        leftMatrix[vertex0][coefIndex]), leftMatrix[vertex1][coefIndex]);
            }
        } else if (leftMatrix[vertex0][0] == null && leftMatrix[vertex1][0] == null) {
            // 0、1为空
            for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
                leftMatrix[vertex0][coefIndex] = zp.createNonZeroRandom(secureRandom);
                leftMatrix[vertex1][coefIndex] = zp.sub(zp.sub(innerProduct[coefIndex],
                        leftMatrix[vertex0][coefIndex]), leftMatrix[vertex2][coefIndex]);
            }
        } else if (leftMatrix[vertex0][0] == null && leftMatrix[vertex2][0] == null) {
            // 0、2为空
            for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
                leftMatrix[vertex0][coefIndex] = zp.createNonZeroRandom(secureRandom);
                leftMatrix[vertex2][coefIndex] = zp.sub(zp.sub(innerProduct[coefIndex],
        ...
        } else if (leftMatrix[vertex1][0] == null && leftMatrix[vertex2][0] == null) {
            // 1、2为空
            for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
                leftMatrix[vertex1][coefIndex] = zp.createNonZeroRandom(secureRandom);
                leftMatrix[vertex2][coefIndex] = zp.sub(zp.sub(innerProduct[coefIndex],
        ...
        } else if (leftMatrix[vertex0][0] == null) {
            // 0为空
            for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
                leftMatrix[vertex0][coefIndex] = zp.sub(zp.sub(innerProduct[coefIndex],
                        leftMatrix[vertex1][coefIndex]), leftMatrix[vertex2][coefIndex]);
            }
        } else if (leftMatrix[vertex1][0] == null) {
            // 1为空
            for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
                leftMatrix[vertex1][coefIndex] = zp.sub(zp.sub(innerProduct[coefIndex],
                        leftMatrix[vertex0][coefIndex]), leftMatrix[vertex2][coefIndex]);
            }
        } else if (leftMatrix[vertex2][0] == null) {
            // 2为空
            for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
                leftMatrix[vertex2][coefIndex] = zp.sub(zp.sub(innerProduct[coefIndex],
                        leftMatrix[vertex0][coefIndex]), leftMatrix[vertex1][coefIndex]);
    ...
    private BigInteger[] innerProduct(final BigInteger[][] zpmVector, final boolean[] binaryVector) {
        assert zpmVector.length == binaryVector.length
                : "Zpm vector length must be equal to binary vector length = " + binaryVector.length + ": " + zpmVector.length;
        BigInteger[] values = new BigInteger[polyDegree];
        Arrays.fill(values, BigInteger.ZERO);
        for (int i = 0; i < zpmVector.length; i++) {
            if (binaryVector[i]) {
                for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
                    values[coefIndex] = zp.add(values[coefIndex], zpmVector[i][coefIndex]);
                }
            }
        }
        return values;
    ...
    private BigInteger[] sub(final BigInteger[] a, final BigInteger[] b) {
        BigInteger[] values = new BigInteger[polyDegree];
        Arrays.fill(values, BigInteger.ZERO);
        for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
            assert zp.validateElement(a[coefIndex]) : "a is not a valid element in Zp: " + a;
            assert zp.validateElement(b[coefIndex]) : "b is not a valid element in Zp: " + b;
            values[coefIndex] = zp.sub(a[coefIndex],b[coefIndex]);
        }
        return values;
    }

//    private BigInteger[] createNonZeroRandom(SecureRandom secureRandom) {
//        BigInteger[] values = new BigInteger[polyDegree];
//        for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++){
//            values[coefIndex] = zp.createNonZeroRandom(secureRandom);
//        }
//        return values;
//    }
private BigInteger[][] generateStorage(Map<T, BigInteger[]> keyValueMap, Set<T> coreDataSet) {
    // 初始化OVDM存储器，所有位置设置为空
    BigInteger[][] storage = new BigInteger[m][polyDegree];
    // Let d˜ = |R| and abort if d˜ > d + λ
    int dTilde = coreDataSet.size();
    // 如果没有2-core边，则补充的边都设置为随机数
    if (dTilde == 0) {
//        System.out.println("dTilde == 0.");
        for (int vertex = lm; vertex < lm+rm; vertex++) {
            for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++) {
                storage[vertex][coefIndex] = zp.createNonZeroRandom(secureRandom);
            }
        }
        return storage;
    }
...
//    }
    ...
    for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++) {
        for (Integer primeIndexC : setPrimeC) {
            storage[primeIndexC][coefIndex] = zp.createNonZeroRandom(secureRandom);
        }
        ...
        for (T data : coreDataSet) {
            ...
            if (storage[h1Value][coefIndex] == null) {
                storage[h1Value][coefIndex] = BigInteger.ZERO;
            }
            if (storage[h2Value][coefIndex] == null) {
                storage[h2Value][coefIndex] = BigInteger.ZERO;
            }
            if (storage[h3Value][coefIndex] == null) {
                storage[h3Value][coefIndex] = BigInteger.ZERO;
            }
            // 3个哈希函数一定互不相同
            mp = zp.add(mp, storage[h1Value][coefIndex]);
            mp = zp.add(mp, storage[h2Value][coefIndex]);
            mp = zp.add(mp, storage[h3Value][coefIndex]);
            for (int rxIndex = 0; rxIndex < rx.length; rxIndex++) {
                if (rx[rxIndex]) {
                    if (storage[lm + rxIndex][coefIndex] == null) {
                        storage[lm + rxIndex][coefIndex] = BigInteger.ZERO;
                    }
                    mp = zp.add(mp, storage[lm + rxIndex][coefIndex]);
                }
            }
            BigInteger value = keyValueMap.get(data)[coefIndex];
        ...
        BigInteger[][] tildeStarMatrixCopy = new BigInteger[dTilde][setC.size()];
        IntStream.range(0, dTilde).forEach(dIndex -> IntStream.range(0,setC.size())
                        .forEach(cIndex -> tildeStarMatrixCopy[dIndex][cIndex] = tildeStarMatrix[dIndex][cIndex]));
        SystemInfo systemInfo = zpLinearSolver.solve(tildeStarMatrixCopy, vectorY, vectorX, true);
        ...
        int xVectorIndex = 0;
        for (int cIndex : setC) {
            storage[lm + cIndex][coefIndex] = vectorX[xVectorIndex];
            xVectorIndex++;
        }
    }
    return storage;
...
//        // 如果没有2-core边，则补充的边都设置为随机数
////        if (dTilde == 0) {
////            IntStream.range(lm, lm + rm).forEach(index -> storage[index] = createNonZeroRandom(secureRandom));
////            return storage;
////        }
//        if (dTilde == 0) {
//            for (int vertex = lm; vertex < lm+rm; vertex++) {
...
//        }
//        // Let M˜' ∈ {0, 1}^{d˜ × (d + λ)} be the sub-matrix of M˜ obtained by taking the row indexed by R.
//        BigInteger[][][] tildePrimeMatrix = new BigInteger[rm][dTilde][polyDegree];
//        int tildePrimeMatrixRowIndex = 0;
//        for (T data : coreDataSet) {
//            boolean[] rxBinary = dataHrMap.get(data);
//            for (int rmIndex = 0; rmIndex < rm; rmIndex++) {
//                for (int coefIndex = 0; coefIndex < polyDegree; coefIndex++) {
//                    tildePrimeMatrix[rmIndex][tildePrimeMatrixRowIndex][coefIndex]
//                            = rxBinary[rmIndex] ? BigInteger.ONE : BigInteger.ZERO;
//                }
//            }
//            tildePrimeMatrixRowIndex++;
