 ...
 */
@Ignore
public class ZpmOvdmEfficiencyTest {
    private static final Logger LOGGER = LoggerFactory.getLogger(ZpmOvdmEfficiencyTest.class);
    /**
     * 最大元素数量对数输出格式
     */
     ...
     * 测试类型
     */
    private static final ZpmOvdmFactory.ZpmOvdmType[] TYPES = new ZpmOvdmFactory.ZpmOvdmType[] {
//            ZpmOvdmFactory.ZpmOvdmType.H2_TWO_CORE_GCT,
//            ZpmOvdmFactory.ZpmOvdmType.H2_SINGLETON_GCT,
            ZpmOvdmFactory.ZpmOvdmType.H3_SINGLETON_GCT,
    };
    ...
    private void testEfficiency(int logN) {
        int n = 1 << logN;
        int polyDegree = 4096;
        for (ZpmOvdmFactory.ZpmOvdmType type : TYPES) {
            ...
            ZpmOvdm<ByteBuffer> ovdm = ZpmOvdmFactory.createInstance(
                    EnvType.STANDARD, type, ZpmOvdmTestUtils.DEFAULT_PRIME, n, keys, polyDegree
            );
            // 生成随机键值对
            Map<ByteBuffer, BigInteger[]> keyValueMap = ZpmOvdmTestUtils.randomKeyValueMap(n, polyDegree);
            STOP_WATCH.start();
