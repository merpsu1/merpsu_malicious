...
import edu.alibaba.mpc4j.s2pc.pso.psu.merpsu22.*;
...
public class PsuFactory implements PtoFactory {
    ...
    public static PsuServer createServer(Rpc serverRpc, Party clientParty, PsuConfig config) {
        ...
        switch (type) {
            ...
            case MERPSU22_OT:
                return new Merpsu22OtPsuServer(serverRpc, clientParty, (Merpsu22OtPsuConfig) config);
            case MERPSU22_AHE:
                return new Merpsu22AhePsuServer(serverRpc, clientParty, (Merpsu22AhePsuConfig) config);
            default:
    ...
    public static PsuClient createClient(Rpc clientRpc, Party serverParty, PsuConfig config) {
        ...
        switch (type) {
            ...
            case MERPSU22_OT:
                return new Merpsu22OtPsuClient(clientRpc, serverParty, (Merpsu22OtPsuConfig) config);
            case MERPSU22_AHE:
                return new Merpsu22AhePsuClient(clientRpc, serverParty, (Merpsu22AhePsuConfig) config);
            default:
                throw new IllegalArgumentException("Invalid " + PsuType.class.getSimpleName() + ": " + type.name());
        }
    }
